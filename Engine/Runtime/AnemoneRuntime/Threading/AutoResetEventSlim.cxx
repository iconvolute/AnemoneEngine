...
{
    AutoResetEventSlim::AutoResetEventSlim(bool signaled)
        : m_Inner{signaled ? StateSignaled : StateReset}
    {
    }

    bool AutoResetEventSlim::Reset()
    {
        return !!this->m_Inner.exchange(StateReset);
    }

    void AutoResetEventSlim::Set()
    {
        this->m_Inner.store(StateSignaled, std::memory_order::release);

        FutexWakeOne(this->m_Inner);
    }

    bool AutoResetEventSlim::IsSignaled() const
    {
        return !!this->m_Inner.load(std::memory_order::acquire);
    }

    void AutoResetEventSlim::Wait()
    {
        while (not this->TryAcquire())
        {
            FutexWait(this->m_Inner, StateReset);
        }
    }

#if false
    bool AutoResetEventSlim::Wait(Duration const& timeout)
    {
    ...
    bool AutoResetEvent::Wait(Duration const& timeout)
    {
        Private::NativeEvent& nativeThis = Platform::Get(this->_native);
