...
namespace Anemone::Internal
{
    struct SHA512Impl final
    {
        ...
        static constexpr void CopyBytesToUInt64BigEndian(uint64_t* destination, size_t count, uint8_t const* source)
        {
            ...
            {
                destination[i] = Bitwise::LoadUnalignedBigEndian<uint64_t>(source + j);
            }
        }
...
namespace Anemone
{
    ...
    std::expected<void, Status> SHA512::Finalize(std::span<std::byte> hash)
    {
        ...
        Bitwise::StoreUnalignedBigEndian<uint64_t>(this->_buffer.data() + 120, length);

        Internal::SHA512Impl::TransformBlock(this->_w, this->_state.data(), this->_buffer.data());

        for (size_t i = 0; i < 8; ++i)
        {
            Bitwise::StoreUnalignedBigEndian<uint64_t>(hash.data() + i * 8, this->_state[i]);
        }
