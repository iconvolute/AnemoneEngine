...
#pragma float_control(pop)
    ...
    RUNTIME_API Half ToHalf(float value);

    RUNTIME_API float FromHalf(Half value);

    RUNTIME_API std::array<Half, 4> ToHalf(std::array<float, 4> const& values);

    RUNTIME_API std::array<float, 4> FromHalf(std::array<Half, 4> const& values);
}
