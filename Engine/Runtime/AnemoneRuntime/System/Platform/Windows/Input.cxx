...
namespace Anemone::Internal::Windows
{
    ...
    void InputImpl::NotifyWindowActivated(WindowImpl& window)
    {
        (void)window;
        this->m_KeyModifiers = CaptureModifiers();
    }

    void InputImpl::NotifyWindowDeactivated([[maybe_unused]] WindowImpl& window)
    {
        (void)window;
        this->m_KeyModifiers = CaptureModifiers();

        this->StopTracking(this->m_TrackMouseWindowHandle);
    }

    void InputImpl::NotifyApplicationActivated()
    {
        this->m_KeyModifiers = CaptureModifiers();
    }

    void InputImpl::NotifyApplicationDeactivated()
    {
        this->m_KeyModifiers = CaptureModifiers();
    ...
    bool InputImpl::StartTracking(HWND handle)
    {
        ...
        {
            this->m_TrackMouseWindowHandle = handle;
            ...
            GetCursorPos(&this->m_TrackMouseRestorePosition);
            this->m_TrackMouseWindowPosition = this->m_TrackMouseRestorePosition;

            MapWindowPoints(nullptr, handle, &this->m_TrackMouseWindowPosition, 1);

            this->m_RawInputLastCursorPosition.x = INT32_MAX;
            this->m_RawInputLastCursorPosition.y = INT32_MAX;
    ...
    void InputImpl::StopTracking(HWND handle)
    {
        if (handle == this->m_TrackMouseWindowHandle && handle != nullptr)
        {
            ...
            this->m_TrackMouseWindowHandle = nullptr;

            // Unclip the cursor
            ClipCursor(nullptr);

            // Restore the cursor position
            SetCursorPos(this->m_TrackMouseRestorePosition.x, this->m_TrackMouseRestorePosition.y);
        }
    }
    ...
    bool InputImpl::ProcessMessage(Interop::WindowMessageResult& result, WindowImpl& window, UINT message, WPARAM wparam, LPARAM lparam)
    {
        ...
        case WM_KEYUP:
        case WM_SYSKEYUP:
            result = this->WmKey(window, wparam, lparam, FALSE);
            break;

        case WM_KEYDOWN:
        case WM_SYSKEYDOWN:
            result = this->WmKey(window, wparam, lparam, TRUE);
            break;

        case WM_INPUT:
            result = this->WmInput(window, wparam, lparam);
            break;

        case WM_LBUTTONDOWN:
            result = this->WmMouseButton(window, VirtualKey::LeftButton, wparam, lparam, TRUE, FALSE);
            break;
            
        case WM_RBUTTONDOWN:
            result = this->WmMouseButton(window, VirtualKey::RightButton, wparam, lparam, TRUE, FALSE);
            break;

        case WM_MBUTTONDOWN:
            result = this->WmMouseButton(window, VirtualKey::MiddleButton, wparam, lparam, TRUE, FALSE);
            break;
        ...
        case WM_LBUTTONUP:
            result = this->WmMouseButton(window, VirtualKey::LeftButton, wparam, lparam, FALSE, FALSE);
            break;

        case WM_RBUTTONUP:
            result = this->WmMouseButton(window, VirtualKey::RightButton, wparam, lparam, FALSE, FALSE);
            break;

        case WM_MBUTTONUP:
            result = this->WmMouseButton(window, VirtualKey::MiddleButton, wparam, lparam, FALSE, FALSE);
            break;
        ...
        case WM_LBUTTONDBLCLK:
            result = this->WmMouseButton(window, VirtualKey::LeftButton, wparam, lparam, FALSE, TRUE);
            break;

        case WM_RBUTTONDBLCLK:
            result = this->WmMouseButton(window, VirtualKey::RightButton, wparam, lparam, FALSE, TRUE);
            break;

        case WM_MBUTTONDBLCLK:
            result = this->WmMouseButton(window, VirtualKey::MiddleButton, wparam, lparam, FALSE, TRUE);
            break;
        ...
        case WM_MOUSEMOVE:
            result = this->WmMouseMove(window, wparam, lparam);
            break;

        case WM_MOUSEWHEEL:
            result = this->WmMouseWheel(window, wparam, lparam);
            break;

        case WM_MOUSEHWHEEL:
            result = this->WmMouseHWheel(window, wparam, lparam);
            break;
    ...
    Interop::WindowMessageResult InputImpl::WmInput(WindowImpl& window, WPARAM wparam, LPARAM lparam)
    {
        ...
        if (code == RIM_INPUT)
        {
            ...
            if (result > 0)
            {
                ...
                POINT& last = this->m_RawInputLastCursorPosition;
                ...
                if (isAbsolute)
                {
                    ...
                    if ((relativeX != 0) || (relativeY != 0))
                    {
                        ...
                        e.Position = Math::PointF{
                            static_cast<float>(m_TrackMouseWindowPosition.x),
                            static_cast<float>(m_TrackMouseWindowPosition.y),
                        };
    ...
    Interop::WindowMessageResult InputImpl::WmMouseButton(WindowImpl& window, VirtualKey virtualKey, WPARAM wparam, LPARAM lparam, BOOL pressed, BOOL click)
    {
        ...
        if (pressed)
        {
            if (click)
            {
                IApplicationEvents::GetCurrent()->OnMouseButtonClick(window, e);
            }
            else
            {
    ...
    MouseButtons InputImpl::ConvertToMouseButtons(UINT keyState)
    {
        MouseButtons result{};
