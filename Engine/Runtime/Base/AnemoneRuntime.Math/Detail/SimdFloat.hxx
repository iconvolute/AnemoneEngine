...
namespace Anemone
{
    ...
    struct VectorF
    {
        using Value = Internal::SimdVector4F;
        using Mask = Internal::SimdMask4F;

        static Value anemone_vectorcall LoadAlignedFloat4(float const source[]) { return _mm_load_ps(source); }
        static Value anemone_vectorcall LoadAlignedFloat3(float const source[]);
        static Value anemone_vectorcall LoadAlignedFloat2(float const source[]);
        static Value anemone_vectorcall LoadAlignedFloat1(float const source[]);

        static void anemone_vectorcall StoreAlignedFloat4(float destination[], Value source) { _mm_store_ps(destination, source); }
        static void anemone_vectorcall StoreAlignedFloat3(float destination[], Value source);
        static void anemone_vectorcall StoreAlignedFloat2(float destination[], Value source);
        static void anemone_vectorcall StoreAlignedFloat1(float destination[], Value source);

        static Value anemone_vectorcall LoadPackedFloat4(float const source[]) { return _mm_loadu_ps(source); }
        static Value anemone_vectorcall LoadPackedFloat3(float const source[]);
        static Value anemone_vectorcall LoadPackedFloat2(float const source[]);
        static Value anemone_vectorcall LoadPackedFloat1(float const source[]);

        static void anemone_vectorcall StorePackedFloat4(float destination[], Value source) { _mm_storeu_ps(destination, source); }
        static void anemone_vectorcall StorePackedFloat3(float destination[], Value source);
        static void anemone_vectorcall StorePackedFloat2(float destination[], Value source);
        static void anemone_vectorcall StorePackedFloat1(float destination[], Value source);

        static Value Add(Value a, Value b) { return _mm_add_ps(a, b); }
        static Value Subtract(Value a, Value b) { return _mm_sub_ps(a, b); }
    };
    ...
    struct Vector4F final
    {
        ...
        static Value anemone_vectorcall Load(Packed const& source)
        {
            return VectorF::LoadPackedFloat4(reinterpret_cast<float const*>(&source));
        }

        static void anemone_vectorcall Store(Packed& destination, Value source)
        {
            VectorF::StorePackedFloat4(reinterpret_cast<float*>(&destination), source);
        }

        static Value anemone_vectorcall Load(Aligned const& source)
        {
            return VectorF::LoadAlignedFloat4(reinterpret_cast<float const*>(&source));
        }

        static void anemone_vectorcall Store(Aligned& destination, Value source)
        {
            VectorF::StoreAlignedFloat4(reinterpret_cast<float*>(&destination), source);
        }

        static Value anemone_vectorcall Add(Value a, Value b)
        {
            return VectorF::Add(a, b);
        }
    };
