...
namespace Anemone
{
    ...
    ANEMONE_RUNTIME_BASE_API void ComputeHistogram(std::vector<uint64_t>& histogram, std::span<uint8_t const> data);

    ANEMONE_RUNTIME_BASE_API void ComputeHistogram(std::vector<uint64_t>& histogram, std::span<float const> data, size_t buckets, float min, float max);

    ANEMONE_RUNTIME_BASE_API double ComputeEntropy(std::span<uint64_t const> histogram, uint64_t total);

    ANEMONE_RUNTIME_BASE_API double ComputeEntropy(std::span<uint64_t const> histogram);
}
