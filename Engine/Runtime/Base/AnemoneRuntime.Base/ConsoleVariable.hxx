...
namespace Anemone
{
    ...
    public:
        ANEMONE_RUNTIME_BASE_API void Register(IConsoleVariable* variable);

        ANEMONE_RUNTIME_BASE_API void Unregister(IConsoleVariable* variable);

        ANEMONE_RUNTIME_BASE_API void Enumerate(FunctionRef<void(IConsoleVariable&)> callback);

        ANEMONE_RUNTIME_BASE_API IConsoleVariable* FindByName(std::string_view name) const;

        ANEMONE_RUNTIME_BASE_API static ConsoleVariableRegistry& Get();
    };
